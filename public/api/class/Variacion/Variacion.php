...
class Variacion extends Mysqli
{
    ...
    public function save(){
        ...
        $query="INSERT INTO variaciones (id, producto, atributo, termino, tipovalor, valora, valor, min, max, base, dependencia, codDependiente,operacion, unidad, orden, alert)"
            ."VALUES (NULL, '".$this->producto."', '".$this->atributo."', '".$this->termino."', '".$this->tipovalor."', '".$this->valora."'"
        .", '".$this->valor."', '".$this->min."', '".$this->max."', '".$this->base."', '".$this->dependencia."','".$this->codDependiente."', '".$this->operacion."', '".$this->unidad."', '".$this->orden."', '".$this->alert."')";
        $save=$this->con->query($query);
        return $save;
    }
    public function getAtributoByProducto($id){
        $resultset=[];
        $query=$this->con->query("SELECT * FROM variaciones WHERE termino=0 AND producto=$id ORDER BY orden");
    ...
    public function deleteByidVariacion($idv){
        $query=$this->con->query("DELETE FROM variaciones WHERE id=$idv");
        return $query;
    }
    ...
    public function update($id){
        ...
        ."operacion='".$this->operacion."',unidad='".$this->unidad."',orden='".$this->orden."' ,alert='".$this->alert."' WHERE id=$id";
